...
const FLOATING_ICONS = [
  { icon: "globe",  top: "12%", left: "8%",   size: 40, dur: 3.2, color: "#3b5bdb" },
  { icon: "rocket", top: "20%", right: "10%", size: 32, dur: 4.0, color: "#0ea5e9" },
  { icon: "target", bottom: "30%", left: "5%", size: 28, dur: 3.6, color: "#6d8bff" },
  { icon: "baby",   top: "55%", right: "6%",  size: 40, dur: 2.8, color: "#38bdf8" },
  { icon: "star",   bottom: "15%", right: "15%", size: 28, dur: 4.4, color: "#3b5bdb" },
  { icon: "school", top: "40%", left: "12%",  size: 32, dur: 3.0, color: "#0ea5e9" },
];
...
const ageGroups = [
  {
    ...
    icon: "baby",
    color: "#3b5bdb",
    gradient: "from-[#3b5bdb] to-[#6d8bff]",
  ...
  },
  {
    ...
    icon: "rocket",
    color: "#6d8bff",
    gradient: "from-[#6d8bff] to-[#9bb2ff]",
...
export default function JuniorPage() {
  ...
  return (
    <>
      {/* Hero */}
      <section className="relative min-h-[92vh] flex items-center overflow-hidden pt-28 pb-20 hero-light">
        <div className="absolute inset-0 grid-lines pointer-events-none opacity-70" />
        <div className="blob blob-royal w-[500px] h-[500px] top-0 left-1/3 pointer-events-none" />
        ...
        {FLOATING_ICONS.map((item, i) => (
          <motion.div
            ...
            style={{ top: item.top, left: (item as { left?: string }).left, right: (item as { right?: string }).right, bottom: (item as { bottom?: string }).bottom }}
          ...
          <div className="grid lg:grid-cols-2 gap-14 items-center">
            ...
            <div className="hidden lg:flex flex-col gap-4">
              {ageGroups.map((g, i) => (
                <motion.div
                  key={g.label}
                  className="card rounded-2xl p-5 flex items-center gap-5"
                  animate={{ y: [0, -10, 0] }}
                  transition={{ duration: 3 + i * 0.6, repeat: Infinity, ease: "easeInOut", delay: i * 0.5 }}
                >
                  <div
                    className="w-14 h-14 rounded-xl flex items-center justify-center flex-shrink-0"
                    style={{ background: `${g.color}14`, border: `1px solid ${g.color}33` }}
                  >
                    <MuiIcon name={g.icon} size={28} style={{ color: g.color }} />
                  </div>
                  <div>
                    <p className="text-ink font-bold text-sm">{g.label}</p>
                    <p className="text-muted text-xs mt-0.5">{g.ages} · {g.highlights[0]}</p>
                  </div>
      ...
      {/* Stats banner */}
      <section className="sec-mist py-10">
        <div className="container-max px-5 md:px-8">
          <div className="card rounded-2xl p-8 border-l-4 border-royal-500 flex flex-col md:flex-row items-center gap-6">
            <div className="relative flex items-center justify-center w-3 h-3">
              <motion.div
                className="w-3 h-3 rounded-full bg-royal-500"
                animate={{ scale: [1, 1.6, 1], opacity: [1, 0.4, 1] }}
                transition={{ duration: 2, repeat: Infinity }}
              />
              <motion.div
                className="absolute w-3 h-3 rounded-full bg-royal-500/30"
                animate={{ scale: [1, 2.5, 1], opacity: [0.5, 0, 0.5] }}
                transition={{ duration: 2, repeat: Infinity }}
              />
            </div>
            <div>
              <p className="text-ink font-black text-xl md:text-2xl">
            ...
            {ageGroups.map((group, i) => (
              <StaggerItem key={group.label}>
                <motion.div
                  ...
                  whileHover={{ y: -6, boxShadow: `0 24px 60px ${group.color}33` }}
                ...
                >
                  ...
                  <div className="p-7">
                    ...
                    >
                      <MuiIcon name={group.icon} size={34} style={{ color: group.color }} />
                    </motion.div>
                    <div
                    ...
                    <h3 className="text-xl font-black text-ink">{group.label}</h3>
                    <p className="text-sm text-muted mt-2 leading-relaxed">{group.desc}</p>
          ...
          <StaggerContainer className="grid sm:grid-cols-2 lg:grid-cols-3 gap-5" staggerDelay={0.08}>
            {whyJunior.map((item, i) => (
              <StaggerItem key={item.title}>
                <div className="card-feature rounded-2xl p-6">
                  <motion.div
          ...
          <div className="max-w-3xl mx-auto">
            <AnimateOnView>
              <div className="card rounded-3xl p-10 text-center relative overflow-hidden">
                <div className="absolute top-0 left-0 w-full h-1.5 bg-gradient-to-r from-[#3b5bdb] to-[#38bdf8]" />
                ...
                <blockquote className="text-xl md:text-2xl text-ink font-medium leading-relaxed italic">
                ...
                <div className="mt-7 flex items-center justify-center gap-3">
                  <div className="w-11 h-11 rounded-full bg-gradient-to-br from-[#3b5bdb] to-[#6d8bff] flex items-center justify-center text-white font-black text-sm">
                    SM
                  </div>
                  <div className="text-left">
                    <p className="text-ink font-semibold text-sm">Sunita Mehta</p>
                    <p className="text-muted text-xs">Parent of Riya (Age 8) · French Little Explorer</p>
                  </div>
                </div>
              </div>
            </AnimateOnView>
      ...
      <section className="section-padding sec-mist">
        <div className="container-max max-w-3xl mx-auto">
          <AnimateOnView className="text-center mb-10">
            ...
            <h2 className="text-3xl font-black text-ink mt-2">Your questions, answered.</h2>
          </AnimateOnView>
          <StaggerContainer className="space-y-4" staggerDelay={0.08}>
            {faqs.map((faq) => (
              <StaggerItem key={faq.q}>
                <div className="card rounded-2xl p-6 border-l-2 border-royal-300">
                  <div className="flex items-start gap-3">
                    <span className="text-xs font-black text-royal-600 bg-royal-50 px-2 py-1 rounded-lg mt-0.5 flex-shrink-0">Q</span>
                    <div>
